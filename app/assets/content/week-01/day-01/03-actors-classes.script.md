...
Next. Q8. What failure modes should you name in an interview? Answer. Class used for DTO → flaky U I after “copy.” Nested class in struct → surprising shared side effects. Assume array assign deep-copies → wrong perf or wrong mental model. Boolean U I state → impossible screens. Actor on every type → contorted APIs. Mutating hashed fields in Set → lost objects. Fix mindset: match type to semantics, audit nested refs, isolate the real concurrent core. Follow-ups. Struct vs class 45s template?: Semantics → mutation → BookMyShow Ads pipeline + HeroWidget lifecycle example: structs for models, classes for UIKit/services.. copy on write 45s template?: Share → uniqueness check → mutate; trust stdlib in hot paths.. Enum state 45s template?: Impossible states out → exhaustiveness → payment popup design..
