...
| Follow-up | Answer |
|---|---|
| Does ATS stop all MITM? | No. A device with a trusted rogue CA can still pass system chain validation. Pinning is an extra identity check on sensitive hosts. |
...
| Follow-up | Answer |
|---|---|
| What does system trust do? | Validates the server cert against Apple’s trusted CA store. |
...
| Follow-up | Answer |
|---|---|
| When might an exception be acceptable? | Short-lived migration from HTTP partner with a dated removal plan — not “we were in a hurry.” |
...
| Follow-up | Answer |
|---|---|
| Soft-fail pinning when? | Low-sensitivity experiments only — never revenue/auth by default. |
