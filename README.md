# iOS Interview Prep

Personalized **senior iOS** interview system for Muhammed Shahid Siddiqui (BookMyShow · District · Raw Engineering).

## Start here

→ **[roadmap/README.md](roadmap/README.md)** — two tracks: Full study + Revision  
→ **[roadmap/weeks/week-01/day-01/README.md](roadmap/weeks/week-01/day-01/README.md)** — Day 01 full chapter  
→ **[app/README.md](app/README.md)** — Flutter audiobook + reader (iOS & Android, Week 1)  
→ **[roadmap/pdf/interview-prep-roadmap.pdf](roadmap/pdf/interview-prep-roadmap.pdf)** — full handbook PDF  

| Path | Purpose |
|---|---|
| [app/](app/) | Audiobook + reader app (TTS, progress, reminders, offline) |
| [roadmap/audio-scripts/README.md](roadmap/audio-scripts/README.md) | Spoken script format |
| [roadmap/revision/](roadmap/revision/) | Short drill notes (after full chapters) |
| [roadmap/stories/story-bank.md](roadmap/stories/story-bank.md) | STAR stories |
| [roadmap/timing/answer-timing-guide.md](roadmap/timing/answer-timing-guide.md) | Answer timing |
| [roadmap/provenance/](roadmap/provenance/) | Verified vs Applied claims |
| [ios-system-design/](ios-system-design/) | Optional deeper SD specs |

```bash
python3 roadmap/scripts/validate_roadmap.py
cd roadmap/pdf && ./generate.sh
```
