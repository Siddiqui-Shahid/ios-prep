...
| Follow-up | Answer |
|---|---|
| Total time? | ~75–90 min with retro — similar muscle to Mock #1. |
| Self mode? | Timer visible; record voice memo; score 1–5. |
| Pick both tracks? | **Forbidden** for 5-min talk — one crisp, one skim after. |

**How can I relate to my case:**
- **Shipped:** Audio streaming + server-driven splash (Aces); BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow backend-driven header & search; BookMyShow SSL pinning + URLSession migration; District Free Parking + Clean/MVVM + AI tooling
...
| Follow-up | Answer |
|---|---|
| Overtime on warm-up? | Trim example; keep definition + trade-off. |
| Two-layer discipline? | Answer points → full spoken compare in 04. |
| Record how many? | Sample suggests 5; mock asks 4–5. |

**How can I relate to my case:**
- **Shipped:** Stories SDK (Raw / Miami Heat); BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow backend-driven header & search
- **Design if asked:** Only if they ask for a modern redesign — label it design, not shipped.
- **Lab only:** N/A for this prompt.
...
| Follow-up | Answer |
|---|---|
| Pin outage? | BookMyShow SSL pinning + URLSession migration Verified controls + Design: pin rotation / break-glass (not shipped runbook) Applied rotation design — not fake runbook. |
| SDUI video player? | Keep native HeroWidget — lifecycle/revenue contract. |
| Dual-stack? | Single router — Hybrid UI / deeplinks; anti-pattern story. |

**How can I relate to my case:**
- **Shipped:** Hybrid UI / deeplinks; BookMyShow SSL pinning + URLSession migration; BookMyShow IMOC + crash-free at scale
...
| Follow-up | Answer |
|---|---|
| Track choice? | Declare Track A (Ads) or B (SDUI) at setup. |
| Record separately? | Yes — architecture block as its own take. |
| Failure modes optional? | Mention if time — don’t skip agenda for detail. |
...
| Follow-up | Answer |
|---|---|
| BookMyShow Ads pipeline + HeroWidget lifecycle time? | 2–3 min STAR; ≤20s elevator also. |
| Design: pin rotation / break-glass (not shipped runbook) label? | Applied design for pin rotation — not shipped runbook. |
| Skip STAR? | Lose behavioral signal — don’t skip when practicing full mock. |

**How can I relate to my case:**
- **Shipped:** Stories SDK (Raw / Miami Heat); Audio streaming + server-driven splash (Aces); Hybrid UI / deeplinks; BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow backend-driven header & search; BookMyShow SSL pinning + URLSession migration; BookMyShow LE Bottom Sheet; District Free Parking + Clean/MVVM + AI tooling
- **Design if asked:** Design: pin rotation / break-glass (not shipped runbook)
- **Lab only:** N/A for this prompt.
- **Don’t claim:** Claiming pin-rotation / break-glass runbook as a shipped production playbook.
...
| Follow-up | Answer |
|---|---|
| Rubric? | Same 1–5 as Mock #1 — timing guide in repo. |
| Top weak cards? | Pin for Week 3 warm-up. |
| Exercises? | Full Mock #2 run in 05-exercises. |

**How can I relate to my case:**
- **Shipped:** BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow backend-driven header & search; BookMyShow SSL pinning + URLSession migration; District Free Parking + Clean/MVVM + AI tooling
- **Design if asked:** Only if they ask for a modern redesign — label it design, not shipped.
- **Lab only:** N/A for this prompt.
- **Don’t claim:** Claiming pin-rotation / break-glass runbook as a shipped production playbook.
...
| Follow-up | Answer |
|---|---|
| Stronger on both? | Still pick one for timed recording; alternate next practice. |
| Bridge answer? | SDUI config + native HeroWidget renderer — best of both in Q&A, not one 5-min mashup. |
| Next files? | [03-ads-architecture.md](03-ads-architecture.md) or [04-sdui-architecture.md](04-sdui-architecture.md). |
...
| Follow-up | Answer |
|---|---|
| Spice BookMyShow SSL pinning + URLSession migration opener? | Alamofire → URLSession, HTTPS, pinning, whitelist (~15–20s). |
| Timing? | ~2:30 total — 20s S/T, ~90s Action, 20–30s Result, 15–20s Lesson. |
| Invent fill-rate? | **Forbidden.** |

**How can I relate to my case:**
- **Shipped:** BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow SSL pinning + URLSession migration
- **Design if asked:** Only if they ask for a modern redesign — label it design, not shipped.
- **Lab only:** N/A for this prompt.
- **Don’t claim:** Claiming pin-rotation / break-glass runbook as a shipped production playbook.
...
| Follow-up | Answer |
|---|---|
| Spice Audio streaming + server-driven splash (Aces)? | Aces live audio + server-driven splash; time-to-interactive (~20–40s). |
| Versioning ask? | Label BookMyShow backend-driven header & search-A1 — version gate + unknown skip as design. |
| Invent splash ms? | **Forbidden.** |
...
**How can I relate to my case:**
- **Shipped:** Stories SDK (Raw / Miami Heat); BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow backend-driven header & search; BookMyShow SSL pinning + URLSession migration; BookMyShow LE Bottom Sheet; District Free Parking + Clean/MVVM + AI tooling
- **Design if asked:** Design: pin rotation / break-glass (not shipped runbook)
- **Lab only:** N/A for this prompt.
- **Don’t claim:** Claiming pin-rotation / break-glass runbook as a shipped production playbook.

Next: [03-ads-architecture.md](03-ads-architecture.md) · [04-sdui-architecture.md](04-sdui-architecture.md)
