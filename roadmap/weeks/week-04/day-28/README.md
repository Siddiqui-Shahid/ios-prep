# Day 28 — Game Day Taper

> Week 4 · Taper / interview readiness (self-contained) · **~2–3 hrs max** then rest 
> Revision twin: [revision/weeks/week-04/day-28.md](../../../revision/weeks/week-04/day-28.md)

## Outcomes

- Weak-area flashcards **only** (Mock #4’s ≤3 fix-forwards + chronic gotchas)
- Priority story bank **once each** with run-through scripts — then stop
- Calm **pre-interview checklist** complete
- **ZERO new topics** — no new LC patterns, WWDC, SD docs, or machine rewrites

## North star

**Protect sleep and confidence. Fresh recall beats last-minute cram.**

## How to use

1. [`01-foundations.md`](01-foundations.md) — taper rules
2. [`02-deep-dive.md`](02-deep-dive.md) — checklist + opener cards (retrieval only)
3. [`03-production-bridge.md`](03-production-bridge.md) — metric trio + honesty
4. [`sample/07-revision-qna.md`](sample/07-revision-qna.md) — light two-layer only (max ~5)
5. [`05-exercises.md`](05-exercises.md) — timed taper protocol + **story run-through scripts**
6. No `code/`


## Module map

| Module | Path |
|---|---|
| Sample Q&A | [sample/README.md](sample/README.md) |

## Forbidden

Entire Anki binge · rewriting STARs from scratch · “just one more Medium” · new SD HLD · caffeine experiments · all-nighter
