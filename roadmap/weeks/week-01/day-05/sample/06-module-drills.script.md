...
Next. Q4. Debugging: intermittent wrong search results? Answer. “Symptom: debounce sleep exists, but Tasks aren’t stored. Root cause: multiple in-flight searches; slow older one wins. Fix: store the Task and cancel it on each keystroke — pattern. Two sentences. Done.” Follow-ups. Still wrong after cancel?: “Add generation token or ‘only apply if query matches.’”.
