# Sample 01 — STAR rhythm & prompt router (Q&A)
...
| Follow-up | Answer |
|---|---|
| What if they want more detail? | Offer one follow-up lane — don’t expand the whole STAR past 4 min. |
| Can Action be shorter? | Yes if Result is strong — but don’t skip the trade-off in Action. |
| Is “Task” separate from Situation? | Often combined in ~20s — “here’s the context and what I owned.” |
...
| Follow-up | Answer |
|---|---|
| What if they say “keep it brief”? | Aim for 90s compressed — S/T + one Action step + Result + Lesson. |
| Should I ask “is that enough detail?” | No — state your plan and execute; they’ll redirect if needed. |
| Behavioral pulled mid-mock? | Same opener — “~2 minutes — context, what I owned, outcome.” |
...
| Follow-up | Answer |
|---|---|
| Two stories fit — which wins? | Pick the one with the strongest metric and clearest *your* ownership. |
| Privacy / on-device AI asked? | **FinTrack on-device AI** or GymFlow on-device AI — not District Free Parking + Clean/MVVM + AI tooling. |
| Backup if BookMyShow Ads pipeline + HeroWidget lifecycle feels thin? | Stories SDK (Raw / Miami Heat) for SDK boundaries. |

**How can I relate to my case:**
- **Shipped:** FinTrack on-device AI; GymFlow on-device AI; Stories SDK (Raw / Miami Heat); Platform upgrade; BookMyShow Ads pipeline + HeroWidget lifecycle; BookMyShow LE Bottom Sheet; BookMyShow IMOC + crash-free at scale; District Free Parking + Clean/MVVM + AI tooling
- **Design if asked:** Only if they ask for a modern redesign — label it design, not shipped.
- **Lab only:** N/A for this prompt.
- **Don’t claim:** Invented fill-rate % or sole credit for ads revenue.
...
| Follow-up | Answer |
|---|---|
| Can I lead with only one metric? | Yes — pick BookMyShow LE Bottom Sheet nav or BookMyShow IMOC + crash-free at scale reliability depending on the question. |
| “Helped drive” vs “owned”? | Honest: you participated in crash-free culture; don’t claim sole CFS ownership. |
| 30L+ DAU — say lakh or million? | Either works — “30+ lakh DAU” or “30L+ DAU” if your resume uses it. |
...
| Follow-up | Answer |
|---|---|
| “Did AI write your code?” | No — tools accelerated boilerplate/tests; I own architecture and reject bad output. |
| When they pivot District Free Parking + Clean/MVVM + AI tooling → FinTrack on-device AI? | Acknowledge the distinction, then answer the product story. |
| Tool-worship red flag? | “AI makes me 10×” with no review bar — senior answer names context engineering. |
...
| Follow-up | Answer |
|---|---|
| “We collaborated well” — enough? | No — need contract, your steps, and a metric or crisp outcome. |
| Can I dramatize an incident? | Process beats drama — stabilize, communicate, prevent. |
| Opinions vs data in conflict? | Contracts and user impact over opinions. |
...
| Follow-up | Answer |
|---|---|
| Include every project? | No — hit scale proof, one specialty, and the senior goal. |
| Repeat full BookMyShow LE Bottom Sheet inside TMAY? | No — cite **30%+** in one clause; save the full STAR for behavioral. |
| Too long at 3 min? | Cut side projects; keep BMS + District + senior target. |
...
| Follow-up | Answer |
|---|---|
| Backend pushback? | Contract fields and content rules — data over opinions. |
| Measure 30%+? | Targeted flows in that scope — not all of BMS nav. |
| Timing check? | ~20s S/T · ~90s Action · ~30s Result · ~20s Lesson. |
...
| Follow-up | Answer |
|---|---|
| Example mitigation? | Feature guard, rollback, or hotfix path — pick one real. |
| Prevent recurrence? | RCA action + checklist/monitoring — not “try harder.” |
| Sole CFS owner? | No — “participated in / helped drive” culture. |
...
Next: [02-conflict-incident.md](02-conflict-incident.md)
